...
/*global google, RichMarker, jQuery, mobile_menu_title, hero_full_screen, slidehow_images, map_color, map_initial_zoom, map_initial_latitude, map_initial_longitude, use_default_map_style, contact_form_success_msg, contact_form_error_msg, contact_form_recaptcha_error_msg, Waypoint, Freewall, map_markers, onepage_nav, grecaptcha  */
...
(function ($) {
	...
	Neela = {
		...
		googleMap: function () {
			...
			if (
				!/^\d|\.|-$/.test($_self.mapInitialLatitude) ||
				!/^\d|\.|-$/.test($_self.mapInitialLongitude)
			) {
				...
				$(window).on('scroll', function () {
					...
					$('.parallax-background').each(function () {
						...
						if (visible) {
							...
							$elem.css(
								'background-position',
								'center ' + parseInt(-(ratio * (height / 250))) + 'px'
							);
						}
					});
				});
				...
				$fields.each(function () {
					...
					if ($field.attr('type') === 'hidden') {
						...
						if (
							...
							$field.val() !== '' &&
							re.test($field.val()) === false
						) {
							$field.addClass('is-invalid');
							error = true;
						} else if (
							$field.attr('id') !== 'g-recaptcha-response' &&
							$field.attr('id') !== 'recaptcha-token'
						) {
								...
								if ($field.attr('type') === 'radio') {
									if ($("input[id='" + $field.attr('id') + "']").is(':checked')) {
										...
										html +=
											...
											len +
											'_value=' +
											encodeHtml($.trim($("label[for='" + $field.attr('id') + "']").text()));
									}
									...
									$('#' + $field.attr('id') + ' input:checkbox:checked').each(function (index) {
										if (index === 0) {
											...
											html += encodeHtml(
												$.trim($("label[for='" + $(this).attr('id') + "']").text())
											);
										} else {
											html +=
												', ' +
												encodeHtml($.trim($("label[for='" + $(this).attr('id') + "']").text()));
										}
									});
								} else {
									html += '&field' + len + '_label=' + encodeHtml($field.attr('name'));
